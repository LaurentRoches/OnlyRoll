...
<template>
  <div class="space-y-6">
    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
      <div class="bg-secondary-800 rounded-lg p-3 text-center">
        ...
        <p class="font-bold text-secondary-100">
          {{
            race.sizeSlug ? t('wiki.reference.creatureSize.' + race.sizeSlug) : (race.size ?? '—')
          }}
        </p>
      </div>
      <div class="bg-secondary-800 rounded-lg p-3 text-center">
        ...
        <p class="font-bold text-secondary-100">
          {{ race.walkSpeed ? race.walkSpeed + ' ft' : '—' }}
        </p>
      </div>
      <div class="bg-secondary-800 rounded-lg p-3 text-center">
    ...
    <div v-if="race.traits?.length">
      <h3 class="text-sm font-medium text-secondary-300 mb-3">
        {{ t('wiki.raceDetail.racialTraits') }}
      </h3>
      <div class="space-y-3">
      ...
      <div class="flex flex-wrap gap-2">
        ...
        >
          ...
          <ChevronDownIcon
            ...
            :class="selectedSubraceId === sr.id ? 'rotate-180' : ''"
          />
        </button>
      </div>
      <div v-if="selectedSubraceId !== null" class="mt-3">
        <div
          v-if="isLoadingSubrace"
        ...
        <div
          v-else-if="subraceDetail"
          class="bg-secondary-800 border border-secondary-600 rounded-lg p-4 space-y-3"
        >
          ...
          <div
            v-if="
              subraceDetail.abilityModifiers && Object.keys(subraceDetail.abilityModifiers).length
            "
          ...
          <p
            v-if="subraceDetail.description"
            class="text-secondary-300 text-sm whitespace-pre-line"
          >
