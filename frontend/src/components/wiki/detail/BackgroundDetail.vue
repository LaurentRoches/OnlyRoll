  ...
  <div class="space-y-4">
    ...
    <div v-if="background.equipment?.length" class="flex flex-wrap gap-2">
      <span class="text-xs text-secondary-400">{{
        t('wiki.backgroundDetail.startingEquipment')
      }}</span>
      <span
        v-for="item in background.equipment"
        :key="item.name"
        class="px-2 py-1 bg-secondary-700 text-secondary-300 rounded text-xs"
      >
