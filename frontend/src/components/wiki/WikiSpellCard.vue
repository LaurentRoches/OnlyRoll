      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span class="text-xs px-1.5 py-0.5 bg-secondary-700 text-secondary-300 rounded">
            {{ spell.level === 0 ? t('wiki.spellCard.cantrip') : t('wiki.spellCard.levelAbbr') + spell.level }}
          </span>
          <span v-if="spell.isConcentration" class="text-xs px-1.5 py-0.5 bg-secondary-700 text-yellow-400 rounded">C</span>
          <span v-if="spell.isRitual" class="text-xs px-1.5 py-0.5 bg-secondary-700 text-green-400 rounded">R</span>
        </div>
