...
import type { PaginatedWikiResponse, MonsterListItem, MonsterDetail, MonsterFilters } from '@/types/wiki'

export const monsterApi = {
  async getMonsters(filters: MonsterFilters = {}): Promise<PaginatedWikiResponse<MonsterListItem>> {
    return get<PaginatedWikiResponse<MonsterListItem>>(`/wiki/monsters${buildWikiQuery(filters as Record<string, string | number | undefined>)}`)
  },
